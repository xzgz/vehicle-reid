...
class HardAttn(nn.Module):
    ...
    def init_params(self):
        ...
        self.fc.bias.data.copy_(torch.tensor([-0.5, -0.5, -0.5, 0.5, 0.5, -0.5, 0.5, 0.5], dtype=torch.float))
...
class HACNN(nn.Module):
    ...
    """
    def __init__(self, num_classes, loss_type='xent', nchannels=[128, 256, 384], feat_dim=512, learn_region=True, use_gpu=True, **kwargs):
    ...
    def forward(self, x):
        ...
        feat_global_local = torch.cat([x_global, x_local], 1)
        feat_global_local = torch.div(feat_global_local, feat_global_local.norm(dim=1, keepdim=True))
        ...
        if not self.training:
            ...
            if self.learn_region:
                ...
                return feat_global_local
            else:
                x_global = torch.div(x_global, x_global.norm(dim=1, keepdim=True))
                return x_global
...
class SoftBlock(nn.Module):
    ...
    def __init__(self, nchannels=[128, 256, 384], input_channel=128, feat_dim=512, **kwargs):
        super(SoftBlock, self).__init__()

        self.inception1 = nn.Sequential(
            InceptionA(input_channel, nchannels[0]),
            InceptionB(nchannels[0], nchannels[0]),
        )
        self.so1 = SoftAttn(nchannels[0])

        self.inception2 = nn.Sequential(
            InceptionA(nchannels[0], nchannels[1]),
            InceptionB(nchannels[1], nchannels[1]),
        )
        self.so2 = SoftAttn(nchannels[1])

        self.inception3 = nn.Sequential(
            InceptionA(nchannels[1], nchannels[2]),
            InceptionB(nchannels[2], nchannels[2]),
        )
        self.so3 = SoftAttn(nchannels[2])
...
class SoftHardBlock(nn.Module):

    def __init__(self, nchannels=[128, 256, 384], input_channel=128, feat_dim=512, **kwargs):
        super(SoftHardBlock, self).__init__()
        self.init_scale_factors()
        self.local_conv1 = InceptionB(input_channel, nchannels[0])
        self.local_conv2 = InceptionB(nchannels[0], nchannels[1])
        self.local_conv3 = InceptionB(nchannels[1], nchannels[2])
        ...
        self.so3 = SoftAttn(nchannels[2])
        self.ha3 = HardAttn(nchannels[2])

        self.fc_global = nn.Sequential(
            nn.Linear(nchannels[2], feat_dim // 2),
            nn.BatchNorm1d(feat_dim // 2),
            nn.ReLU(),
        )

        self.fc_local = nn.Sequential(
            nn.Linear(nchannels[2] * 4, feat_dim // 2),
            nn.BatchNorm1d(feat_dim // 2),
            nn.ReLU(),
        )
    ...
    def forward(self, x):
        ...
        x1_attn = self.so1(x1)
        x1_out = x1 * x1_attn
        x1_theta = self.ha1(x1)
        x1_local_list = []
        for region_idx in range(4):
            x1_theta_i = x1_theta[:, region_idx, :]
            x1_theta_i = self.transform_theta(x1_theta_i, region_idx)
            x1_trans_i = self.stn(x, x1_theta_i)
            x1_trans_i = F.upsample(x1_trans_i, (20, 20), mode='bilinear', align_corners=True)
            x1_local_i = self.local_conv1(x1_trans_i)
            x1_local_list.append(x1_local_i)
        ...
        x2_attn = self.so2(x2)
        x2_out = x2 * x2_attn
        x2_theta = self.ha2(x2)
        x2_local_list = []
        for region_idx in range(4):
            x2_theta_i = x2_theta[:, region_idx, :]
            x2_theta_i = self.transform_theta(x2_theta_i, region_idx)
            x2_trans_i = self.stn(x1_out, x2_theta_i)
            x2_trans_i = F.upsample(x2_trans_i, (10, 10), mode='bilinear', align_corners=True)
            x2_local_i = x2_trans_i + x1_local_list[region_idx]
            x2_local_i = self.local_conv2(x2_local_i)
            x2_local_list.append(x2_local_i)
        ...
        x3_attn = self.so3(x3)
        x3_out = x3 * x3_attn
        x3_theta = self.ha3(x3)
        x3_local_list = []
        for region_idx in range(4):
            x3_theta_i = x3_theta[:, region_idx, :]
            x3_theta_i = self.transform_theta(x3_theta_i, region_idx)
            x3_trans_i = self.stn(x2_out, x3_theta_i)
            x3_trans_i = F.upsample(x3_trans_i, (5, 5), mode='bilinear', align_corners=True)
            x3_local_i = x3_trans_i + x2_local_list[region_idx]
            x3_local_i = self.local_conv3(x3_local_i)
            x3_local_list.append(x3_local_i)
