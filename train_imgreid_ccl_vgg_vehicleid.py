...
from torchreid.dataset_loader import ImageDataset, BalancedBatchSampler, CCLBatchSampler
from torchreid.losses.triplet_loss import OnlineTripletLossV2
...
from torchreid.evaluate_vehicleid import cmc_vehicleid
from torchreid.losses.gste_loss import GSTELoss
from torchreid.utils.olccl_loader import VehicleIdCCLBatchSampler, VehicleIdCCLBatchSampler
from torchreid.utils.olccl_loss import OnlineCCLTripletLoss, CoupledClustersLoss
from torchreid.utils.olccl_utils import RandomNegativeTripletSelector, CCLRandomNegativeTripletSelector
...
parser.add_argument('--data_dir',
                    default='/home/gysj/pytorch-workspace/pytorch-study/data/VehicleID_V1.0/vehicleid',
                    type=str,
...
if use_gpu_suo:
    ...
    # args.save_dir = '/home/weiying1/hyg/pytorch-workspace/pytorch-study/log-reid/vggm1024-angle-veri776-bh64-lr1ef1'
    # args.save_dir = '/home/weiying1/hyg/pytorch-workspace/pytorch-study/log-reid/vggm1024-tripletrnt-veri776-bh64-lr1ef4'
    # args.save_dir = '/home/weiying1/hyg/pytorch-workspace/pytorch-study/log-reid/vggm1024-ccl-veri776-bh128-lr1ef5'
    args.save_dir = '/home/weiying1/hyg/pytorch-workspace/pytorch-study/log-reid/vggm1024-xtl-veri776-bh64-lr1ef5'
else:
    args.root = '/home/gysj/pytorch-workspace/pytorch-study/data'
    # args.save_dir = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-angle-veri776-bh64-lr1ef2'
    # args.save_dir = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-xent-veri776-bh64-lr1ef1'
    # args.save_dir = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-triplethnt-veri776-bh64-lr1ef5'
    # args.save_dir = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-xtl-veri776-bh64-lr1ef5'
    # args.save_dir = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-xent-vs-xtl-veri776-bh64-lr1ef2'
    args.save_dir = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-ccl-vehicleid-bh6-5-lr1ef4'

    # args.resume = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-xent-veri776-bh64-lr1ef1/best_model.pth.tar'
    # args.resume = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-xent-veri776-bh64-lr1ef2/best_model.pth.tar'
    # args.resume = '/media/sda1/sleep-data/gysj/log-reid/vggm1024-olccl-vehicleid-bh64-lr1ef4/best_model.pth.tar'

args.arch = 'vggm1024'
args.dataset = 'veri776'
args.gpu_devices = '1'
args.workers = 4
...
def main():
    ...
    train_dataset = ImageFolder(os.path.join(args.data_dir, "train_all"), transform=transform_train)
    train_query_dataset = ImageFolder(os.path.join(args.data_dir, "val"), transform=transform_train)
    train_gallery_dataset = ImageFolder(os.path.join(args.data_dir, "train"), transform=transform_train)
    test_dataset = ImageFolder(os.path.join(args.data_dir, "probe"), transform=transform_test)
    query_dataset = ImageFolder(os.path.join(args.data_dir, "query"), transform=transform_test)
    gallery_dataset = ImageFolder(os.path.join(args.data_dir, "gallery"), transform=transform_test)
    ...
    train_batch_sampler = VehicleIdCCLBatchSampler(train_dataset, n_classes=n_cls, n_samples=n_samples)

    trainloader = DataLoader(
        train_dataset, batch_sampler=train_batch_sampler, num_workers=args.workers, pin_memory=pin_memory
    )

    testloader = DataLoader(test_dataset, batch_size=args.test_batch, shuffle=False,
    ...
    queryloader = DataLoader(
        query_dataset, batch_size=args.test_batch, shuffle=False, num_workers=args.workers,
        pin_memory=pin_memory, drop_last=False,
    )

    galleryloader = DataLoader(
        gallery_dataset, batch_size=args.test_batch, shuffle=False, num_workers=args.workers,
        pin_memory=pin_memory, drop_last=False,
    )

    train_query_loader = DataLoader(
        train_query_dataset, batch_size=args.test_batch, shuffle=False, num_workers=args.workers,
        pin_memory=pin_memory, drop_last=False,
    )

    train_gallery_loader = DataLoader(
        train_gallery_dataset, batch_size=args.test_batch, shuffle=False, num_workers=args.workers,
        pin_memory=pin_memory, drop_last=False,
    )
    ...
    if args.evaluate:
        model = models.init_model(name=args.arch, num_classes=len(query_dataset.classes),
                                  loss_type=args.loss_type)
    else:
        model = models.init_model(name=args.arch, num_classes=len(train_dataset.classes),
                                  loss_type=args.loss_type)
    print("Model size: {:.3f} M".format(count_num_param(model)))

    if args.label_smooth:
        criterion = CrossEntropyLabelSmooth(num_classes=len(train_dataset.classes), use_gpu=use_gpu)
    else:
        if args.loss_type == 'xent':
            ...
            criterion = CoupledClustersLoss(margin=1., n_classes=n_cls, n_samples=n_samples)
            ...
            criterion = XentTripletLoss(margin=1., triplet_selector=RandomNegativeTripletSelector(margin=1.))
        else:
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        if (epoch + 1) > args.start_eval and args.eval_step > 0 and (
            ...
            print("==> Test")
            rank1 = test(model, testloader, queryloader, galleryloader, train_query_loader, train_gallery_loader, use_gpu)
    ...
    for batch_idx, (imgs, pids) in enumerate(trainloader):
        ...
        if isinstance(outputs, tuple):
            if args.loss_type == 'xent':
                ...
                loss = criterion(outputs[0], outputs[1], pids)
            else:
                raise KeyError("Unsupported loss: {}".format(args.loss_type))
        else:
            loss = criterion(outputs, pids)
...
def test(model, testloader, queryloader, galleryloader, train_query_loader, train_gallery_loader, use_gpu, ranks=[1, 5, 10, 20],
